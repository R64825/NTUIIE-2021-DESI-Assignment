...
namespace Random_Variate_Generator_Library
{
    public class Discrete_Event_Simulation_Engine
    {
        ...
        public Discrete_Event_Simulation_Engine(List<Server> list_Of_Server, List<TimedQueue> list_Of_Queue, Client_Generator client_Generator, Service_Node service_Node)
        {
        ...
        {
            string str = $"Simulation end at {Math.Round(last_Event_Time, 2) }\n";
            str += client_Generator.Get__Description();
            //str += service_Node.Get__Description();
            str += "\n    [Servers]\n\n";
            for (int i = 0; i < list_Of_Server.Count; i++)
            {
