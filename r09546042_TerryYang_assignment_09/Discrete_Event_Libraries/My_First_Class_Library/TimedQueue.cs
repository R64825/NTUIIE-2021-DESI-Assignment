...
namespace Random_Variate_Generator_Library
{
    public class TimedQueue
    {
        ...
        public TimedQueue(Service_Node node, int index)
        {
            ...
            this.index = index;
            Name = $"Queue_0{index}";
            series_Queue.Name = Name;
            series_Queue.ChartType = SeriesChartType.StepLine;
            rnd = new Random(index);
            //double p = rnd.NextDouble();
            series_Queue.Color = Color.FromArgb(Convert.ToInt32( 255 * rnd.NextDouble()), Convert.ToInt32(255 * rnd.NextDouble()), Convert.ToInt32(25 * rnd.NextDouble()));
        ...
        {
            return $"Queue_0{index } -> \n" +
                $"    maximum {maximum_Clients_in_Queue} clients in this queue,\n" +
